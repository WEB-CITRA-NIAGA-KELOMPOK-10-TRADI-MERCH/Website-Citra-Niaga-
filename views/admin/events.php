...
<body class="bg-[#f4f7f6] text-slate-800 h-screen overflow-hidden flex">
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-white">
        ...
        <header class="bg-white px-8 py-4 flex justify-between items-center border-b border-slate-100 z-10 shrink-0">
            <div>
                <h1 class="font-bold text-xl text-slate-800 tracking-wider uppercase">Kelola Acara</h1>
                <p class="text-xs text-slate-400">Atur acara, festival, dan live music di kawasan</p>
            </div>
        ...
        <main class="flex-1 overflow-y-auto p-8 hide-scrollbar bg-slate-50/50 relative">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mb-8">
                <div class="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
                    <i data-lucide="calendar-plus" class="text-blue-600 w-5 h-5"></i>
                ...
                <form action="../../controllers/EventController.php" method="POST" enctype="multipart/form-data" class="space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="title" required placeholder="Misal: Festival Mahakam 2026" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition">
                        </div>
                        
                        <div>
                            <label class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 block">Tanggal Mulai</label>
                            <input type="date" name="start_date" required class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition cursor-pointer">
                        </div>
                        
                        <div>
                            <label class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 block">Tanggal Selesai</label>
                            <input type="date" name="end_date" required class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition cursor-pointer">
                        </div>

                        <div class="md:col-span-2">
                            <label class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 block">Deskripsi Acara</label>
                            <textarea name="description" rows="3" required placeholder="Ceritakan detail acara ini..." class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition"></textarea>
                        </div>

                        <div class="md:col-span-2">
                            <label class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 block">Upload Poster / Banner</label>
                            <input type="file" name="image" accept="image/jpeg, image/png, image/webp" required class="block w-full text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition cursor-pointer border border-slate-200 rounded-xl bg-slate-50">
                            <p class="text-[10px] text-slate-400 mt-2">Rekomendasi format: JPG, PNG, WEBP. Maksimal ukuran 2MB.</p>
                        </div>
                    </div>

                    <div class="pt-4 flex justify-end">
                        <button type="submit" name="tambah_event" class="bg-blue-600 text-white px-6 py-2.5 rounded-xl font-bold hover:bg-blue-700 transition shadow-md flex items-center gap-2">
            ...
            <div class="flex flex-wrap gap-2 mb-6" id="filter-container">
                <button onclick="filterEvents('all', this)" class="filter-btn px-5 py-2.5 rounded-full text-sm font-bold bg-blue-600 text-white shadow-sm border border-blue-600 transition-all">Semua Acara</button>
                                ...
                                <tr class="event-row hover:bg-slate-50/80 transition duration-150" data-category="<?= $status ?>">
                                    ...
                                    <td class="py-4 px-6 align-middle">
                                        <div class="inline-flex flex-col gap-2 items-stretch min-w-[170px]">
                                            ...
                                            <div class="flex items-center justify-center gap-1.5 text-xs font-bold text-blue-600 bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100 <?= $status == 'past' ? 'bg-slate-50 border-slate-200 text-slate-500' : '' ?>">
                                    ...
                                    <td class="py-4 px-6 text-center space-x-2">
                                        <button type="button" onclick="document.getElementById('editModal<?= $row['id'] ?>').classList.remove('hidden')" class="inline-block bg-blue-50 hover:bg-blue-100 text-blue-600 p-2 rounded-lg transition border border-blue-100" title="Edit Event">
                                    ...
                                    <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
                                        <div class="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                                            <h3 class="font-bold text-lg text-slate-800 flex items-center gap-2">
                                                <i data-lucide="edit-3" class="w-5 h-5 text-blue-600"></i> Edit <?= htmlspecialchars($row['title']) ?>
                                            </h3>
                                        ...
                                        <div class="p-6 overflow-y-auto">
                                            <form action="../../controllers/EventController.php" method="POST" enctype="multipart/form-data" class="space-y-4 text-left">
                                                ...
                                                <div>
                                                    ...
                                                    <input type="text" name="title" value="<?= htmlspecialchars($row['title']) ?>" required class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition">
                                                </div>
                                                
                                                <div class="grid grid-cols-2 gap-4">
                                                    <div>
                                                        <label class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1 block">Tanggal Mulai</label>
                                                        <input type="date" name="start_date" value="<?= $row['start_date'] ?>" required class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition">
                                                    </div>
                                                    <div>
                                                        <label class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1 block">Tanggal Selesai</label>
                                                        <input type="date" name="end_date" value="<?= $row['end_date'] ?>" required class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition">
                                                    </div>
                                                </div>

                                                <div>
                                                    <label class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1 block">Deskripsi Acara</label>
                                                    <textarea name="description" rows="4" required class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition"><?= htmlspecialchars($row['description']) ?></textarea>
                                                </div>

                                                <div>
                                                    <label class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1 block">Upload Poster Baru (Opsional)</label>
                                                    <input type="file" name="image" accept="image/jpeg, image/png, image/webp" class="block w-full text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition cursor-pointer border border-slate-200 rounded-xl bg-slate-50">
                                                    <p class="text-[10px] text-slate-400 mt-2">Biarkan kosong jika tidak ingin mengubah poster saat ini.</p>
                                                </div>
                                                
                                                <div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
                                                    <button type="button" onclick="document.getElementById('editModal<?= $row['id'] ?>').classList.add('hidden')" class="px-5 py-2.5 rounded-xl font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 transition">Batal</button>
                                                    <button type="submit" name="edit_event" class="bg-blue-600 text-white px-6 py-2.5 rounded-xl font-bold hover:bg-blue-700 transition shadow-md">Simpan Perubahan</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
    ...
    <script>
        ...
        function filterEvents(category, btnElement) {
            ...
            // 2. Saring baris tabel berdasarkan atribut data-category
