...
$sidebar_text = !empty($setting['admin_sidebar_text_color']) ? htmlspecialchars($setting['admin_sidebar_text_color']) : '#cbd5e1';
$admin_text_color = !empty($setting['admin_text_color']) ? htmlspecialchars($setting['admin_text_color']) : '#1e293b';
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <style>
        :root {
            ...
            --admin-text: <?= $admin_text_color ?>;
            --admin-shadow: color-mix(in srgb, var(--admin-theme) 30%, transparent);
        }
        body { font-family: 'Plus Jakarta Sans', sans-serif; }
        
        body, .text-slate-800, .text-gray-800, .text-slate-700, .text-gray-700, h1, h2, h3, h4, h5, h6 { 
            color: var(--admin-text) !important; 
        }

        aside, aside span, aside i, .text-slate-300, .text-slate-400 {
            color: var(--sidebar-text) !important;
        }
        aside a.bg-theme span, aside a.bg-theme i {
...
<body class="bg-gray-50 flex h-screen overflow-hidden relative">
    ...
    <aside id="main-sidebar" class="w-64 bg-sidebar text-slate-300 flex flex-col h-full shrink-0 shadow-2xl z-40 fixed lg:static transition-transform duration-300 ease-in-out -translate-x-full lg:translate-x-0">
        <div class="p-6 flex items-center justify-between border-b border-white/5">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[10px]" style="color: var(--sidebar-text) !important; opacity: 0.7;">Admin Portal</p>
                </div>
            </div>
            <button onclick="toggleSidebar()" class="lg:hidden p-1"><i data-lucide="x" class="w-6 h-6"></i></button>
        </div>
        ...
        <nav class="flex-1 px-4 py-4 space-y-2 overflow-y-auto hide-scrollbar">
            ...
            <a href="gallery.php" class="flex items-center gap-3 px-4 py-3 rounded-xl transition <?= ($admin_page == 'gallery.php') ? 'bg-theme text-white shadow-theme' : 'hover:bg-white/5' ?>">
                <i data-lucide="image" class="w-5 h-5"></i> <span class="text-sm font-medium">Kelola Galeri</span>
            </a>
            <a href="history.php" class="flex items-center gap-3 px-4 py-3 rounded-xl transition <?= ($admin_page == 'history.php') ? 'bg-theme text-white shadow-theme' : 'hover:bg-white/5' ?>">
                <i data-lucide="book-open" class="w-5 h-5"></i> <span class="text-sm font-medium">Kelola Sejarah</span>
            </a>
            <a href="kios.php" class="flex items-center gap-3 px-4 py-3 rounded-xl transition <?= ($admin_page == 'kios.php') ? 'bg-theme text-white shadow-theme' : 'hover:bg-white/5' ?>">
                <i data-lucide="store" class="w-5 h-5"></i> <span class="text-sm font-medium">Kelola Kios</span>
            </a>
            <a href="events.php" class="flex items-center gap-3 px-4 py-3 rounded-xl transition <?= ($admin_page == 'events.php') ? 'bg-theme text-white shadow-theme' : 'hover:bg-white/5' ?>">
                <i data-lucide="calendar" class="w-5 h-5"></i> <span class="text-sm font-medium">Kelola Acara</span>
            </a>
            <a href="reviews.php" class="flex items-center gap-3 px-4 py-3 rounded-xl transition <?= ($admin_page == 'reviews.php') ? 'bg-theme text-white shadow-theme' : 'hover:bg-white/5' ?>">
                <i data-lucide="message-square" class="w-5 h-5"></i> <span class="text-sm font-medium">Kelola Ulasan</span>
            </a>
            <a href="settings.php" class="flex items-center gap-3 px-4 py-3 rounded-xl transition <?= ($admin_page == 'settings.php') ? 'bg-theme text-white shadow-theme' : 'hover:bg-white/5' ?>">
                <i data-lucide="settings" class="w-5 h-5"></i> <span class="text-sm font-medium">Pengaturan Web</span>
            </a>
        </nav>
        ...
        <div class="p-4 mt-auto border-t border-white/5">
            <a href="../../controllers/AuthController.php?logout=true" class="flex items-center gap-3 px-4 py-3 text-red-400 hover:bg-red-400/10 rounded-xl transition font-medium">
                <i data-lucide="log-out" class="w-5 h-5"></i> <span class="text-sm">Logout</span>
            </a>
        </div>
    </aside>
    ...
    <main class="flex-1 flex flex-col h-screen min-w-0 overflow-hidden bg-slate-50/50">
        <header class="bg-white border-b border-gray-200 px-4 lg:px-8 py-4 sticky top-0 z-10 flex justify-between items-center shadow-sm shrink-0">
            <div class="flex items-center gap-3">
                <button onclick="toggleSidebar()" class="lg:hidden p-2 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200 transition"><i data-lucide="menu" class="w-5 h-5"></i></button>
                <div>
                    <h2 class="text-lg lg:text-xl font-bold uppercase tracking-wider">PENGATURAN WEB</h2>
            ...
            <form action="../../controllers/SettingsController.php" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 lg:p-8 form-container">
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 pb-4 border-b border-gray-100 gap-3">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-bold text-lg">Form Konfigurasi Utama</h3>
                    </div>
                    <button type="button" id="btnResetAll" class="text-xs bg-red-50 hover:bg-red-100 text-red-600 px-3 py-2 rounded-lg transition-colors flex items-center gap-1.5 font-bold border border-red-100 w-full sm:w-auto justify-center">
                        <i data-lucide="alert-triangle" class="w-4 h-4"></i> Reset Total Semua
                    </button>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-1">
                            <label class="text-sm font-semibold block">Judul Website (SEO Title)</label>
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-1">
                            <label class="text-sm font-semibold block">Deskripsi Singkat (SEO Description)</label>
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-1">
                            <label class="text-sm font-semibold block">Kata Kunci (SEO Keywords)</label>
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-200">
                        <div class="flex items-center gap-3 mb-6">
                            <i data-lucide="monitor" class="text-theme w-5 h-5"></i>
                            <h3 class="font-bold text-lg">Warna Tampilan Admin</h3>
                        </div>
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-4">
                                <p class="text-xs font-bold text-gray-400 uppercase tracking-widest">Bagian Sidebar</p>
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="bg-slate-50 p-4 rounded-xl border border-slate-200" id="color_container_adminSidebar">
                                        <label class="text-xs font-semibold block mb-2">Latar Sidebar</label>
                                        <div class="flex items-center gap-3">
                                            <input type="color" id="adminSidebarColorInput" name="admin_sidebar_color" value="<?= $sidebar_color ?>" class="w-8 h-8 rounded cursor-pointer border-0 p-0 shadow-sm">
                                            <p class="font-bold text-xs" id="adminSidebarColorText"><?= strtoupper($sidebar_color) ?></p>
                                        </div>
                                        <button type="button" onclick="resetColor('adminSidebar', '#1e293b')" class="text-[10px] text-blue-500 mt-2 hover:underline flex items-center gap-1"><i data-lucide="rotate-ccw" class="w-3 h-3"></i> Reset</button>
                                    </div>
                                    <div class="bg-slate-50 p-4 rounded-xl border border-slate-200" id="color_container_adminSidebarText">
                                        <label class="text-xs font-semibold block mb-2">Teks Sidebar</label>
                                        <div class="flex items-center gap-3">
                                            <input type="color" id="adminSidebarTextColorInput" name="admin_sidebar_text_color" value="<?= $sidebar_text ?>" class="w-8 h-8 rounded cursor-pointer border-0 p-0 shadow-sm">
                                            <p class="font-bold text-xs" id="adminSidebarTextColorText"><?= strtoupper($sidebar_text) ?></p>
                                        </div>
                                        <button type="button" onclick="resetColor('adminSidebarText', '#cbd5e1')" class="text-[10px] text-blue-500 mt-2 hover:underline flex items-center gap-1"><i data-lucide="rotate-ccw" class="w-3 h-3"></i> Reset</button>
                                    </div>
                                </div>
                            </div>

                            <div class="space-y-4">
                                <p class="text-xs font-bold text-gray-400 uppercase tracking-widest">Bagian Konten</p>
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="bg-slate-50 p-4 rounded-xl border border-slate-200" id="color_container_adminTheme">
                                        <label class="text-xs font-semibold block mb-2">Warna Aksen</label>
                                        <div class="flex items-center gap-3">
                                            <input type="color" id="adminThemeColorInput" name="admin_theme_color" value="<?= $admin_theme ?>" class="w-8 h-8 rounded cursor-pointer border-0 p-0 shadow-sm">
                                            <p class="font-bold text-xs" id="adminThemeColorText"><?= strtoupper($admin_theme) ?></p>
                                        </div>
                                        <button type="button" onclick="resetColor('adminTheme', '#2563eb')" class="text-[10px] text-blue-500 mt-2 hover:underline flex items-center gap-1"><i data-lucide="rotate-ccw" class="w-3 h-3"></i> Reset</button>
                                    </div>
                                    <div class="bg-slate-50 p-4 rounded-xl border border-slate-200" id="color_container_adminText">
                                        <label class="text-xs font-semibold block mb-2">Teks Konten</label>
                                        <div class="flex items-center gap-3">
                                            <input type="color" id="adminTextColorInput" name="admin_text_color" value="<?= $admin_text_color ?>" class="w-8 h-8 rounded cursor-pointer border-0 p-0 shadow-sm">
                                            <p class="font-bold text-xs" id="adminTextColorText"><?= strtoupper($admin_text_color) ?></p>
                                        </div>
                                        <button type="button" onclick="resetColor('adminText', '#1e293b')" class="text-[10px] text-blue-500 mt-2 hover:underline flex items-center gap-1"><i data-lucide="rotate-ccw" class="w-3 h-3"></i> Reset</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-200">
                        <div class="flex items-center gap-3 mb-6">
                            <i data-lucide="globe" class="text-theme w-5 h-5"></i>
                            <h3 class="font-bold text-lg">Warna Tampilan Publik</h3>
                        </div>

                        <p class="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3 mt-4">Warna Latar Belakang & Aksen</p>
                        
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                            <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 transition-all" id="color_container_theme">
                                <label class="text-xs font-semibold block mb-2">Warna Utama (Aksen)</label>
                                <div class="flex flex-col gap-2">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <p class="font-bold text-xs" id="themeColorText"><?= strtoupper(htmlspecialchars($setting['theme_color'] ?? '#254794')) ?></p>
                                    </div>
                                    <button type="button" onclick="resetColor('theme', '#254794')" class="text-[10px] bg-white border border-gray-300 hover:bg-gray-200 px-2 py-1.5 rounded-lg transition-colors flex items-center justify-center gap-1 shadow-sm font-medium w-full mt-1"><i data-lucide="rotate-ccw" class="w-3 h-3"></i> Default</button>
                                </div>
                            </div>
                            <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 transition-all" id="color_container_header">
                                <label class="text-xs font-semibold block mb-2">Latar Header</label>
                                <div class="flex flex-col gap-2">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <p class="font-bold text-xs" id="headerColorText"><?= strtoupper(htmlspecialchars($setting['header_color'] ?? '#ffffff')) ?></p>
                                    </div>
                                    <button type="button" onclick="resetColor('header', '#ffffff')" class="text-[10px] bg-white border border-gray-300 hover:bg-gray-200 px-2 py-1.5 rounded-lg transition-colors flex items-center justify-center gap-1 shadow-sm font-medium w-full mt-1"><i data-lucide="rotate-ccw" class="w-3 h-3"></i> Default</button>
                                </div>
                            </div>
                            <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 transition-all" id="color_container_footer">
                                <label class="text-xs font-semibold block mb-2">Latar Footer</label>
                                <div class="flex flex-col gap-2">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <p class="font-bold text-xs" id="footerColorText"><?= strtoupper(htmlspecialchars($setting['footer_color'] ?? '#1e293b')) ?></p>
                                    </div>
                                    <button type="button" onclick="resetColor('footer', '#1e293b')" class="text-[10px] bg-white border border-gray-300 hover:bg-gray-200 px-2 py-1.5 rounded-lg transition-colors flex items-center justify-center gap-1 shadow-sm font-medium w-full mt-1"><i data-lucide="rotate-ccw" class="w-3 h-3"></i> Default</button>
                                </div>
                            </div>
                        </div>

                        <p class="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3 mt-4">Warna Teks</p>
                            ...
                            <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 transition-all" id="color_container_text">
                                ...
                                <div class="flex flex-col gap-2">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <p class="font-bold text-xs" id="textColorText"><?= strtoupper(htmlspecialchars($setting['text_color'] ?? '#333333')) ?></p>
                                    </div>
                                    <button type="button" onclick="resetColor('text', '#333333')" class="text-[10px] bg-white border border-gray-300 hover:bg-gray-200 px-2 py-1.5 rounded-lg transition-colors flex items-center justify-center gap-1 shadow-sm font-medium w-full mt-1"><i data-lucide="rotate-ccw" class="w-3 h-3"></i> Default</button>
                                </div>
                            </div>
                            <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 transition-all" id="color_container_headerText">
                                <label class="text-xs font-semibold block mb-2">Teks Header</label>
                                <div class="flex flex-col gap-2">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <p class="font-bold text-xs" id="headerTextColorText"><?= strtoupper(htmlspecialchars($setting['header_text_color'] ?? '#333333')) ?></p>
                                    </div>
                                    <button type="button" onclick="resetColor('headerText', '#333333')" class="text-[10px] bg-white border border-gray-300 hover:bg-gray-200 px-2 py-1.5 rounded-lg transition-colors flex items-center justify-center gap-1 shadow-sm font-medium w-full mt-1"><i data-lucide="rotate-ccw" class="w-3 h-3"></i> Default</button>
                                </div>
                            </div>
                            <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 transition-all" id="color_container_footerText">
                                <label class="text-xs font-semibold block mb-2">Teks Footer</label>
                                <div class="flex flex-col gap-2">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <p class="font-bold text-xs" id="footerTextColorText"><?= strtoupper(htmlspecialchars($setting['footer_text_color'] ?? '#9ca3af')) ?></p>
                                    </div>
                                    <button type="button" onclick="resetColor('footerText', '#9ca3af')" class="text-[10px] bg-white border border-gray-300 hover:bg-gray-200 px-2 py-1.5 rounded-lg transition-colors flex items-center justify-center gap-1 shadow-sm font-medium w-full mt-1"><i data-lucide="rotate-ccw" class="w-3 h-3"></i> Default</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                            <div class="bg-slate-50 p-5 rounded-xl border border-slate-200 transition-all">
                                <div class="flex justify-between items-center mb-2">
                                    <label class="text-sm font-semibold block">Jenis Font Publik</label>
                            ...
                            <div class="bg-slate-50 p-5 rounded-xl border border-slate-200 transition-all" id="banner_container">
                                <div class="flex justify-between items-center mb-2">
                                    <label class="text-sm font-semibold block">Ganti Banner Utama</label>
                                ...
                                <input type="file" id="input_hero_banner" name="hero_banner" accept="image/jpeg, image/png, image/webp" class="block w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold bg-white text-theme transition-all cursor-pointer border border-slate-200 mt-1">
                                <p id="banner_status_text" class="text-[10px] mt-2 mb-2 transition-colors text-gray-500">Biarkan kosong jika tidak mengubah banner (Maks 2MB)</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-200">
                        <div class="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
                            ...
                            <h3 class="font-bold text-lg">Pengaturan Kontak</h3>
                        </div>
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <div class="flex justify-between items-center mb-1">
                                    <label class="text-sm font-semibold block"><i data-lucide="phone" class="w-4 h-4 inline pb-0.5"></i> No. Telepon / WA</label>
                                    <button type="button" onclick="resetField('input_contact_phone', '+62 822-1949-7715')" class="text-[10px] text-blue-500 hover:text-blue-700 flex items-center gap-1 font-medium bg-blue-50 px-2 py-0.5 rounded transition-colors"><i data-lucide="rotate-ccw" class="w-3 h-3"></i> Default</button>
                                </div>
                            ...
                            <div>
                                <div class="flex justify-between items-center mb-1">
                                    <label class="text-sm font-semibold block"><i data-lucide="mail" class="w-4 h-4 inline pb-0.5"></i> Alamat Email</label>
                                    <button type="button" onclick="resetField('input_contact_email', 'info@citraniaga.com')" class="text-[10px] text-blue-500 hover:text-blue-700 flex items-center gap-1 font-medium bg-blue-50 px-2 py-0.5 rounded transition-colors"><i data-lucide="rotate-ccw" class="w-3 h-3"></i> Default</button>
                                </div>
                            ...
                            <div>
                                <div class="flex justify-between items-center mb-1">
                                    <label class="text-sm font-semibold block"><i data-lucide="instagram" class="w-4 h-4 inline pb-0.5"></i> Username Instagram</label>
                                    <button type="button" onclick="resetField('input_contact_ig', '@citraniagasamarinda')" class="text-[10px] text-blue-500 hover:text-blue-700 flex items-center gap-1 font-medium bg-blue-50 px-2 py-0.5 rounded transition-colors"><i data-lucide="rotate-ccw" class="w-3 h-3"></i> Default</button>
                                </div>
                            ...
                            <div class="md:col-span-2">
                                <div class="flex justify-between items-center mb-1">
                                    <label class="text-sm font-semibold block"><i data-lucide="map-pin" class="w-4 h-4 inline pb-0.5"></i> Alamat Lengkap Kawasan</label>
                                    <button type="button" onclick="resetField('input_contact_address', 'Jl. Niaga Selatan, Kel. Pelabuhan, Kec. Samarinda Kota, Kota Samarinda, Kalimantan Timur 75112')" class="text-[10px] text-blue-500 hover:text-blue-700 flex items-center gap-1 font-medium bg-blue-50 px-2 py-0.5 rounded transition-colors"><i data-lucide="rotate-ccw" class="w-3 h-3"></i> Default</button>
                                </div>
    ...
    <script>
        ...
        function setBannerDefault() {
            ...
            st.textContent = "⚠️ Banner akan dikembalikan ke bawaan setelah disimpan.";
        ...
        function resetFile(inputId) {
            ...
            const st = document.getElementById('banner_status_text');
            st.textContent = "Biarkan kosong jika tidak mengubah banner (Maks 2MB)";
        ...
        setupColorPicker('adminSidebarText');
        setupColorPicker('adminTheme');
        setupColorPicker('adminText');
        ...
        document.getElementById('btnResetAll').addEventListener('click', function() {
            if(confirm('Yakin ingin mereset TOTAL seluruh pengaturan ke bawaan pabrik?')) {
                ...
                resetColor('adminText', '#1e293b');
                resetColor('adminSidebar', '#1e293b'); 
                resetColor('adminSidebarText', '#cbd5e1'); 
                ...
                resetField('input_contact_phone', '+62 822-1949-7715');
                resetField('input_contact_email', 'info@citraniaga.com');
                resetField('input_contact_ig', '@citraniagasamarinda');
                resetField('input_contact_address', 'Jl. Niaga Selatan, Kel. Pelabuhan, Kec. Samarinda Kota, Kota Samarinda, Kalimantan Timur 75112');
                
                setBannerDefault();

                alert("Semua form telah dikembalikan ke data default. JANGAN LUPA KLIK 'SIMPAN SEMUA PERUBAHAN'!");
            }
        });
